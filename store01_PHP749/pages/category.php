...
$category = $res->fetch(); // Мы достаем одну категорию, поэтому используем не fetchAll, а просто fetch
...
$products = $res->fetchAll(); // Мы достаем все продукты, поэтому используем fetchAll, а не просто fetch
                ...
                <div class='card-body'>
                    ...
                    <div class='card-basket-buttons'>
                        <form action='/actions/basket/remove_product.php' method='POST' >
                            ...
                            <input name='category_id' hidden value='{$product['category_id']}'> <!--Для возможности вернуться обратно-->
